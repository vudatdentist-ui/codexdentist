...
- Public odontogram prototype: `https://odontogram.codexdentist.com`; it provides separate FDI charts for 32 permanent teeth and 20 primary teeth, using generated anatomical SVG assets from the pinned MIT-licensed `@artidev/odontogram-core` package, five independently selectable surfaces, separate crown/root marking, and stackable clinical symbols for pulpal, periodontal, implant, endodontic, restorative, missing/extraction, and fracture findings. Prototype state is browser-local and is not yet a patient clinical record.
