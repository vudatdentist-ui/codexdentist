...
The cache rule `Cache public odontogram shell` applies only to `odontogram.codexdentist.com/` with a one-hour edge TTL. The app response for that host/root must keep `Cache-Control: public, max-age=0, must-revalidate`: Cloudflare may retain the edge copy, but browsers must revalidate the shell after a release. After each shared-host deployment, purge that exact URL in Cloudflare, request it once to warm the new HTML, then verify a non-empty `MISS` followed by non-empty `HIT` responses. Do not broaden this rule to authenticated app routes or patient data.
