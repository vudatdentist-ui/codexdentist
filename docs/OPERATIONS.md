...
The cache rule `Cache public odontogram shell` applies only to `odontogram.codexdentist.com/` with a one-hour edge TTL. The app response for that host/root must keep `Cache-Control: no-store, max-age=0, must-revalidate`: the Cloudflare rule may retain its forced edge copy, but browsers must not retain the shell. This avoids the four-hour browser TTL injected by LiteSpeed for otherwise cacheable responses. After each shared-host deployment, purge that exact URL in Cloudflare, request it once to warm the new HTML, then verify a non-empty `MISS` followed by non-empty `HIT` responses. Do not broaden this rule to authenticated app routes or patient data.
