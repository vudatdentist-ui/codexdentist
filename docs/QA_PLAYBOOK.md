...
- Odontogram anatomy check: crown/root zones are clicked directly on the current tooth artwork without rendering a second tooth image; each zone keeps its own condition, works for upper/lower permanent and primary teeth, persists in `anatomyState`, and is included in undo/reset/export. Missing teeth and implants clear/disable both zones, and one undo restores the previous anatomy state.
