...
- `https://odontogram.codexdentist.com/` keeps the root URL, switches between 32 permanent and 20 primary FDI teeth, renders every generated anatomical SVG asset without fallback or distortion, exposes five surfaces through keyboard controls, supports compatible simultaneous clinical symbols per tooth, retains the bone/gum block for missing teeth, lowers that block when bone loss is added, labels the occlusion control `Khớp cắn`, persists its summaries for both arches and each arch, keeps tooth marks isolated by dentition, and has no horizontal overflow at `390x844`.
