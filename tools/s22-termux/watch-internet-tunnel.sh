...
WATCH_LOG="$TERMUX_HOME/codexmed-tunnel-watchdog.log"
...
log "Cloudflare tunnel watchdog started."

while true; do
  if ! tunnel_alive; then
    log "Cloudflare tunnel is down; starting a quick tunnel."
    cd "$APP_DIR"
    bash tools/s22-termux/start-internet-tunnel.sh >> "$WATCH_LOG" 2>&1 || true
  fi
