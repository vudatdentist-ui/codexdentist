  ...
  try {
    const [patient, catalogItem] = await Promise.all([
      ...
      prisma.serviceCatalogItem.findFirst({
        ...
        include: { defaultCompensationRule: true },
      }),
    ]);

    if (!patient || !catalogItem) redirect(serviceRedirect("journey-service-missing"));

    await prisma.$transaction(async (tx) => {
      const existingCount = await tx.treatmentService.count({
        where: { patientId, serviceCatalogItemId },
      });
      ...
      for (const [index, target] of targets.entries()) {
        ...
        await tx.treatmentService.create({
          data: {
            ...
            compensationRuleVersion: catalogItem.defaultCompensationRule?.version ?? null,
      ...
      await tx.patientJourneyState.updateMany({
        ...
        data: {
          odontogramTeeth: [],
          odontogramSnapshot: { selectedTargets: [] } as Prisma.InputJsonValue,
    ...
    await prisma.auditLog.create({
      data: {
        ...
        entityId: patientId,
        metadata: { serviceCatalogItemId, targets, diagnosis } as Prisma.InputJsonValue,
      },
    });
  } catch (error) {
...
export async function updateJourneyTreatmentServiceDiscountAction(formData: FormData) {
  const session = await requireViewSession("journey");
  if (!canPerformAction(session, "treatment.plan.create")) redirect("/journey?notice=journey-denied");
  ...
  const serviceRedirect = (notice: string) => journeyRedirect(notice, patientId);
  if (!treatmentServiceId) redirect(serviceRedirect("journey-service-missing"));
  ...
  try {
    ...
    if (!service) redirect(serviceRedirect("journey-service-missing"));
    ...
    await prisma.$transaction([
      prisma.treatmentService.update({ where: { id: service.id }, data: { finalPrice } }),
...
export async function deleteJourneyTreatmentServiceAction(formData: FormData) {
  const session = await requireViewSession("journey");
  if (!canPerformAction(session, "treatment.service.delete")) redirect("/journey?notice=journey-denied");
  ...
  try {
    const service = await prisma.treatmentService.findFirst({
      ...
      select: {
        ...
        receiptAllocations: { select: { id: true } },
        progressEvents: { select: { id: true }, take: 1 },
        compensationAccruals: { select: { id: true }, take: 1 },
      },
    });
    if (!service) redirect(serviceRedirect("journey-service-missing"));
    ...
    if (hasClinicalOrFinancialHistory) redirect(serviceRedirect("journey-service-delete-locked"));

    await prisma.$transaction([
      prisma.treatmentService.delete({ where: { id: service.id } }),
      prisma.auditLog.create({
        data: {
          organizationId: session.organizationId,
          actorId: databaseActorId(session.userId),
...
export async function recordJourneyServiceProgressAction(formData: FormData) {
  const session = await requireViewSession("journey");
  if (!canPerformAction(session, "treatment.service.progress")) redirect("/journey?notice=journey-denied");
  ...
  const performedById = requiredString(formData.get("performedById")) || session.userId;
    ...
    await runSerializableTransaction(async (tx) => {
      const service = await tx.treatmentService.findFirst({
        ...
        include: {
          ...
          compensationRule: { include: { pools: { include: { shares: true } } } },
        },
      });
      if (!service) redirect(progressRedirect("journey-progress-missing"));

      const consultantId = consultantFieldPresent ? requestedConsultantId : service.createdById;
        ...
        where: {
          ...
          roleAssignments: {
            some: {
              active: true,
              role: { not: "PATIENT" },
              OR: [{ clinicId: null }, { clinicId: service.clinicId }],
            },
          },
        },
      });
      ...
      if (toProgressPercent < fromProgressPercent) redirect(progressRedirect("journey-progress-regression"));
      const progressDeltaPercent = Math.max(toProgressPercent - fromProgressPercent, 0);
      const nextStatus = toProgressPercent >= 100 ? "COMPLETED" : toProgressPercent > 0 ? "IN_PROGRESS" : "PLANNED";
      const nextStepSequence =
        service.serviceCatalogItem?.steps
          ...
          ?.sequence ?? null;
      const rule = service.compensationRule ? ruleInputFromDatabase(service.compensationRule) : defaultServiceCompensationRule;
      const compensation = progressDeltaPercent > 0
        ? calculateServiceProgressCompensation({
...
async function consumeServiceMaterials({
  ...
  service: {
    ...
    serviceName: string;
    serviceCatalogItem: {
      steps?: Array<{ sequence: number; defaultProgress: unknown }>;
      materials: Array<{
        ...
        inventoryItem: {
          ...
          lots: Array<{ id: string; quantityOnHand: unknown }>;
        } | null;
      }>;
    } | null;
  };
  ...
  for (const material of materials) {
    if (!material.inventoryItemId || material.quantity == null) continue;
    ...
    for (const lot of material.inventoryItem?.lots ?? []) {
      ...
      await tx.inventoryLot.update({
        where: { id: lot.id },
        data: { quantityOnHand: { decrement: consumedFromLot } },
      });
    ...
    await tx.inventoryItem.update({
      ...
      data: { onHandQuantity: { decrement: quantity } },
    });
...
function ruleInputFromDatabase(rule: {
  pools: Array<{
    ...
    shares: Array<{ role: string; sharePercent: unknown; fallbackRole: string | null }>;
  }>;
  ...
  return {
    ...
    fallbackMissingClinicalSupportToOperator: Boolean(
      doctorPool?.shares.find((share) => share.role === "CLINICAL_SUPPORT")?.fallbackRole,
    ),
    fallbackMissingAssistantPrimaryToOperator:
      assistantPool?.shares.find((share) => share.role === "ASSISTANT_PRIMARY")?.fallbackRole === "OPERATOR",
    fallbackMissingAssistantSecondaryToPrimary: Boolean(
      assistantPool?.shares.find((share) => share.role === "ASSISTANT_SECONDARY")?.fallbackRole,
    ),
  };
}

function sharePercent(
  pool: { shares: Array<{ role: string; sharePercent: unknown }> } | undefined,
  role: string,
) {
...
function patientCodeFromId(patientId: string) {
  ...
  const sequence = numericId ? Number(numericId.slice(-6)) : stableNumberFromText(patientId, 999999);
