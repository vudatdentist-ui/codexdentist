...
import { PrescriptionTemplatePrintActions } from "@/components/PrescriptionTemplatePrintActions";
import { requireViewSession } from "@/lib/auth";
import { getPrintablePrescriptionTemplate } from "@/lib/pharmacy";

export default async function PrescriptionTemplatePrintPage({
  params,
  searchParams,
}: {
  ...
  const prescription = await getPrintablePrescriptionTemplate(
    session,
    ...
    <main className="prescription-print-page">
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .print-actions a, .print-actions button { min-height: 34px; border-radius: 6px; border: 1px solid #9ca3af; background: white; color: #111827; padding: 0 12px; font-weight: 700; text-decoration: none; }
      ...
      <PrescriptionTemplatePrintActions />
      ...
      <header className="clinic-head">
        ...
        <div className="rx-meta">
          <p>Ngày in: {prescription.createdAt}</p>
        </div>
      </header>
      ...
      <section className="patient-grid">
        ...
        <div className="patient-cell">
          ...
          <p>{[prescription.patientDateOfBirth, prescription.patientAge ? `${prescription.patientAge} tuổi` : ""].filter(Boolean).join(" - ") || "................"}</p>
        </div>
        <div className="patient-cell">
          ...
          {prescription.items.map((item, index) => (
            <tr key={item.id}>
              ...
              <td>
                ...
                <div className="drug-use">{item.sig}</div>
              </td>
